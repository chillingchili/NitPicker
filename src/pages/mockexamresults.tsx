...
export default function MockExamResultsPage() {
  ...
  const [openWrongQuestions, setOpenWrongQuestions] = useState<
        ...
        <div className="flex-1 w-full flex items-center justify-center px-6 md:px-20">
          <div className="border-2 border-black dark:border-white p-8 md:p-10 w-full max-w-xl flex flex-col items-center gap-6 text-center">
            ...
            <p className="text-sm md:text-base opacity-75">
              Take a mock exam first so this page can display your score and
              breakdown.
            </p>
            <button
              type="button"
  ...
  const averageTimePerQuestion = formatClockFromSeconds(
    result.averageTimePerQuestionSeconds,
    false,
  );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-start">
            <section className="flex flex-col items-stretch lg:items-start px-2 sm:px-4 md:px-10">
              <div className="flex items-end gap-3 sm:gap-5">
                ...
                <h1 className="font-semibold text-2xl sm:text-3xl text-black opacity-50 dark:text-white">
                  /{result.totalQuestions}
                </h1>
              </div>
              <p
                className={`mt-3 text-lg sm:text-xl font-bold ${didPass ? "text-green-700" : "text-red-700"}`}
              >
                {didPass
                  ? `We did it bro (${PASSING_PERCENTAGE}% or higher)`
                  : `NT tried bro (below ${PASSING_PERCENTAGE}%)`}
              </p>
              ...
              <div className="mt-6 md:mt-8 flex flex-col gap-4 md:gap-5 w-full lg:max-w-sm">
                ...
                <div className="flex items-baseline justify-between border-b pb-2">
                  ...
                  <p
                    className={`text-2xl sm:text-3xl font-bold ${didPass ? "text-green-700" : "text-red-700"}`}
                  >
              ...
              <div className="mt-8 flex flex-col gap-3 w-full lg:max-w-md">
                ...
                <div className="flex items-baseline justify-between border-b pb-2 gap-4">
                  ...
                  <p className="text-lg sm:text-xl font-bold text-right">
                    {result.settings.examType}
                  </p>
                </div>
                <div className="flex items-baseline justify-between border-b pb-2 gap-4">
                  <p className="text-sm opacity-70">Timer</p>
                  <p className="text-lg sm:text-xl font-bold text-right">
                    {result.settings.isTimed ? "Timed" : "Not timed"}
                  </p>
                </div>
                <div className="flex items-baseline justify-between border-b pb-2 gap-4">
                  <p className="text-sm opacity-70">Duration</p>
                  <p className="text-lg sm:text-xl font-bold text-right">
                    {result.settings.isTimed
                      ? `${result.settings.durationMinutes} mins`
                      : "No limit"}
                  </p>
                </div>
                <div className="flex items-baseline justify-between border-b pb-2 gap-4">
                ...
                <div className="flex items-baseline justify-between border-b pb-2 gap-4">
                  ...
                  <p className="text-lg sm:text-xl font-bold text-right">
                    {result.settings.instantAnswers ? "Instant" : "No reveal"}
                  </p>
                </div>
                <div className="flex items-baseline justify-between border-b pb-2 gap-4">
                  <p className="text-sm opacity-70">Categories</p>
                  <p className="text-lg sm:text-xl font-bold text-right">
                    {result.selectedCategories.length === 3
                      ? "All categories"
                      : result.selectedCategories.join(", ")}
                  </p>
                </div>
                <div className="flex items-baseline justify-between border-b pb-2 gap-4">
                              ...
                              <ul className="mt-3 flex flex-col gap-2 text-sm md:text-base opacity-75">
                                {category.topics.map((topic) => (
                                  <li
                                    ...
                                    className="flex items-start sm:items-center justify-between gap-2 sm:gap-4 border-b border-black/10 pb-1"
                                  >
                  ...
                  <div className="flex flex-col gap-4">
                    {wrongQuestions.map((wrongQuestion, index) => {
                      const isOpen = Boolean(
                        openWrongQuestions[wrongQuestion.id],
                      );

                      return (
                        <article
                          key={wrongQuestion.id}
                          className="border-2 border-black/20 dark:border-white/20 bg-white dark:bg-zinc-900"
                        >
                          <button
                            type="button"
                            onClick={() =>
                              toggleWrongQuestion(wrongQuestion.id)
                            }
                            ...
                            {isOpen ? (
                              ...
                              >
                                <div className="px-3 md:px-4 pb-4 border-t border-black/10 dark:border-white/10 flex flex-col gap-3">
                                  ...
                                  <div
                                    className="border-l-4 border-black pl-3 max-h-72 overflow-y-auto
                                    ...
                                    <div style={{ direction: "ltr" }}>
                                      ...
                                      <div className="mt-1 text-sm md:text-base leading-relaxed prose prose-sm max-w-none prose-p:my-1">
                                        {wrongQuestion.answerExplanation ? (
                                          ...
                                          <p>
                                            No detailed explanation available in
                                            the source markdown for this item.
                                          </p>
                                        )}
                                      </div>
                                    </div>
